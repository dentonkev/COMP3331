...
<img src="./images/ip2.png" width="450" height="auto">

<img src="./images/ip3.png" width="450" height="auto">

<img src="./images/ip4.png" width="450" height="auto">
...
- $D$: data bits 
- $G$: bit pattern 
- $r$: CRC bits = number of bits in $G$ - 1 
- $R$: remainder, ($D *2^r$) / $G$

We have data $D$ and known value $G$. Start by appending $r$ $0$ bits to $D$, where $r$ = num bits $G$ - 1, then divide $D$ by $G$ to get remainder $R$ that has $r$ bits. Append the remainder $R$ to $D$, giving $<D,R>$, such that $<D,R>$ exactly divisible by $G$ (mod 2). 
...
- Channels CANNOT send simultaneously.
- If there is a rate R and N nodes, then bandwidth for all users is R / N, even if some users  are idle (user 2,5,6 in example).
  - Even if only 1 user, rate still R / N.
...
- Channels CAN send simultaneously (**better than TDMA**). 
- If there is a rate R and N nodes, then bandwidth for all users is R / N, even if some users  are idle (bands 2 and 5 in example).
  - Even if only 1 user, rate still R / N.
...
**If there is a rate R and N nodes, then bandwidth for all users is NOT R / N, since there are collisions. However, if there is only 1 user, rate is R.**
...
- After aborting, enter binary (exponential backoff)
  - after mth collision choose K at random from {0, 1, 2, ... 2^m - 1} and wait K * 512 bit times before next transmission.
- Efficiency: better performance than ALOHA: and simple, cheap, decentralized.

<img src="./images/csmacd.png" width="300" height="auto">

## Taking turns Protocol
- Nodes take turns, but nodes with more to send can take longer turns
- If there is a rate R and N nodes, then bandwidth for all users is R / N, however some users transfer more time to transfer, but on averge R / N. 
- If only 1 user, rate is R.

**Polling:** Captain node “invites” other nodes to transmit in turn.

<img src="./images/poll.png" width="300" height="auto">

### Token Passing:
Control token passed from one node to next sequentially. Computers are arranged in a "ring".
...
Example: A wants to send datagram to B. It sends out a broadcast with B's IP and MAC address **FF-FF-FF-FF**. B responds with a message directly to A with its MAC address and A adds it into its ARP table.
...
<img src="./images/svr.png" width="550" height="auto">
...
<img src="./images/ber.png" width="300" height="auto">
...
### AP Scanning
<img src="./images/scn.png" width="550" height="auto">
